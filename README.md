...
* sql server allows to use some keywords as table or column names, but the parser this project uses can be confused by that. For instance, if your CTE is named "temp",
  ...
  There will be some attempt to work around it but it may not be possible in all cases if there is too much ambiguity.
  Another solution may be to modify the parser itself.
