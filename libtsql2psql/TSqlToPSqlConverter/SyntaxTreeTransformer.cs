...
public class SyntaxTreeTransformer {
    ...
    private void AddMissingSemicolons(Node element)
    {
        ...
        if (element.Name == SqlStructureConstants.ENAME_MERGE_CLAUSE) return;
    ...
    /// </summary>
    /// <param name="element"></param>
    private void ForceMergeInto(Node element)
    {
        if (element.Matches(SqlStructureConstants.ENAME_MERGE_TARGET))
        {
            var intoKeyword = element.ChildByNameAndText(SqlStructureConstants.ENAME_OTHERKEYWORD, "into");
            if (intoKeyword != null)
            {
                return;
            }

            element.InsertChildBefore(SqlStructureConstants.ENAME_OTHERKEYWORD, "into", element.Children.First());
        }

        foreach (var child in new List<Node>(element.Children))
        {
            ForceMergeInto(child);
        }
    }
    ...
    private void ConvertDateDiffFunction(Node element)
    {
        if (element.Matches(SqlStructureConstants.ENAME_FUNCTION_KEYWORD, "datediff")) {
            ...
            var interval = parens.ChildrenByName(SqlStructureConstants.ENAME_OTHERNODE).First();
