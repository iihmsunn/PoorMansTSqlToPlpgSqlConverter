...
{
    public class TSqlToPsqlConverter : ISqlTreeFormatter
    {
        public TSqlToPsqlConverter() : this(new TSqlToPsqlConverterOptions()) { }
        
        public TSqlToPsqlConverter(TSqlToPsqlConverterOptions options)
        {
            if (options == null)
        ...
        public TSqlToPsqlConverterOptions Options { get; private set; }
        ...
        private void ProcessSqlNode(Node contentElement, TSqlStandardFormattingState state)
        {
            ...
            switch (contentElement.Name)
            {
				...
				case SqlStructureConstants.ENAME_IN_PARENS:
                    ...
                    //psql specific
                    var closestClause = contentElement.Closest(SqlStructureConstants.ENAME_SQL_CLAUSE);
                    if (contentElement.Name == SqlStructureConstants.ENAME_DDL_PARENS && closestClause?.ChildByName(SqlStructureConstants.ENAME_SEMICOLON) == null)
                    {
                        state.BreakExpected = true;
                        state.UnIndentInitialBreak = true;
                    }
